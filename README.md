...
```bash
# 下載並執行安裝腳本
git clone http://fw_git.phison.com/kyle_lei/teammate.git ~/.teammate-hub
~/.teammate-hub/install.sh /path/to/your-project
```

或者如果你已在目標專案目錄：

```bash
~/.teammate-hub/install.sh .
```

安裝完成後即可使用 `/teammate.init` 開始。

### 方式二：從 Teammate Hub 直接同步

若你已有 Teammate Hub clone，在目標專案執行：

```bash
TEAMMATE_HOME=/path/to/Teammate bash /path/to/Teammate/dist/skills/teammate/scripts/bash/teammate-sync.sh .
```

### 更新框架

```bash
~/.teammate-hub/install.sh /path/to/your-project --self-update
```
...
```
/teammate.skills              # 查詢、安裝、更新 Skills
/teammate.sync                # 同步框架到消費專案 / 檢查版本
```
...
| **Skills** | `/teammate.skills` | 查詢、安裝、更新 Skills 與 Agents |
| **Sync** | `/teammate.sync` | 同步框架版本到消費專案 |
...
Teammate 採「**精簡文件生成規則**」而非廢棄文件——保留可追溯性、降低 token 消耗：
...
│
├── .cursor/
│   ├── commands/                # Teammate 指令（teammate.* 生命週期）
│   │   ├── teammate.init.md
│   │   ├── teammate.align.md
│   │   ├── teammate.plan.md
│   │   ├── teammate.execute.md
...
│   │   └── ...
│   └── rules/
│       └── teammate.mdc         # 共創契約與架構原則（alwaysApply）
│
└── tasks/                       # 任務工作目錄
    └── [###-task-name]/
...
- `.teammate/design/` 為設計資產目錄，當 `context.md` 包含 Figma URL 時由 `/teammate.align` 自動建立。
- `.cursor/agents/` 為 AI 角色目錄，依使用者選擇按需載入，不佔用常駐 context。
- `.cursor/skills/` 為領域知識與能力模組，由 Agent 依意圖路由載入；可透過 `/teammate.skills` 管理。
