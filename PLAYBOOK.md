...
- **依日期排序**：新增筆數時須按 **日期由新到舊** 插入在正確位置，不得隨意安插（同一天多筆則按加入先後或類型排列）
...
**決策**: 
1. `teammate.review.md` 在「下一步行動」下新增小節「任務結束且無需修正時（務必執行）」：當 Ready 且無 CRITICAL/HIGH 時，報告結尾**必須**詢問「是否要 commit 並 merge 回 main？」
2. 說明：若要則在 Agent 模式下請 AI 代為執行 commit → checkout main → merge；若否可稍後自行操作。註明 review 為唯讀，不代為執行 Git，僅提醒流程：Review 完成 → commit → checkout main → merge
3. Report Completion 增加條款：Readiness 為 Ready 且無需修正時，輸出結尾必須含上述詢問
4. `teammate.helpme.md` 在「Review 完成」情境補充版控流程說明，與 review 的提醒一致
**影響**: `.cursor/commands/teammate.review.md`、`.cursor/commands/teammate.helpme.md`
**成效**: 任務就緒時 AI 主動詢問是否 commit/merge，版控收尾不遺漏
**狀態**: 生效中
...
#### 2026-02-14 · 技能 · oReady
**觸發**: pyairtable SDK 使用中文 Table 名稱呼叫 Meta API（`create_field`）時，URL 編碼導致 Airtable 回傳 404。反覆嘗試用中文名稱、URL encode、不同 API 路徑均失敗，浪費多輪對話。
**決策**: 先用 `base.schema()` 取得所有 Table 的 ID（如 `tblsX6c1y0pXSMzo5`），再用 `api.table(base_id, table_id)` 操作 Meta API。一般讀寫（`all()`、`batch_create()`）用中文名稱正常，**只有 Meta API 必須用 Table ID**。
**影響**: `stock_agent/sync/airtable_syncer.py`、`tasks/012-weinstein-stage-analysis/insights.md`
**成效**: 使用 Table ID 後一次成功建立所有欄位
**狀態**: 生效中
...
#### 2026-02-13 · 架構 · OpenWebUI_Frontend
...
#### 2026-02-13 · 命令 · OpenWebUI_Frontend
...
#### 2026-02-10 · 架構 · OpenWebUI_Frontend
...
#### 2026-02-10 · 命令 · OpenWebUI_Frontend
...
#### 2026-02-09 · 規則 · OpenWebUI_Frontend
...
#### 2026-02-09 · 命令 · OpenWebUI_Frontend
