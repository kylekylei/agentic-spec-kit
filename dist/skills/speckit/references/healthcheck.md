...
description: 工作流程健康診斷。執行 5 個 Pass 掃描 Foundation 完整性、階段合規、artifact 庫存、追溯骨架、Hub 同步狀態，產出 Healthcheck Report。
---
...
若 `speckit.yml` 有 `hub.url` 或 `speckitsync_rule.mdc` 存在，讀取 Hub 來源，檢查：
- Hub 路徑是否可讀？
- `speckit-rules.mdc` 與 Hub 版本是否一致？
- `speckit.*.md` commands 與 Hub 版本是否一致？
