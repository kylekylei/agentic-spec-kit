...
description: 框架版本遷移。比對 Hub 與專案版本，產出遷移報告，確認後套用更新。不碰專案私有檔案。
---

# Migrate

**定位**：版本遷移 —— 比對專案的框架版本與 Hub 最新版本，產出遷移計畫並套用更新。
...
2. 若無，讀取 `{ide}/rules/speckitsync_rule.mdc` 的 `**Hub**: \`[PATH]\``（本機路徑）
3. 若為 URL：clone 至 `.specify/hub-cache/speckit` 後使用；若為本機路徑：驗證存在且可讀
