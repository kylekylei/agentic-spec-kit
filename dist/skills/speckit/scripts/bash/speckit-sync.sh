#!/usr/bin/env bash
# speckit-sync.sh — Sync speckit dist/ to consumer projects
#
# Usage:
...
Environment:
  SPECKIT_HOME    Path to speckit repo (default: auto-detected from script location)
                   Fallback: read hub.url from .specify/config/speckit.yml (clone to cache)
EOF
}

# ── Read hub.url from speckit.yml ──
...
# Resolution order: 1) speckit.yml hub.url (clone to cache) 2) speckitsync_rule.mdc
resolve_speckit_home() {
    # If dist/ already exists at SPECKIT_HOME, we're good
    [[ -d "$SPECKIT_HOME/dist" ]] && return 0

    # 1. Try speckit.yml hub.url first
    ...
    for yml in "${yml_candidates[@]}"; do
        ...
        if [[ -n "$hub_url" ]]; then
            ...
            if [[ -d "$cache_dir/dist" ]]; then
                ...
                DIST_DIR="$SPECKIT_HOME/dist"
                warn "SPECKIT_HOME resolved from speckit.yml hub.url (cached) → $SPECKIT_HOME"
                return 0
            fi
            ...
            if git clone --depth 1 "$hub_url" "$cache_dir" 2>/dev/null; then
                SPECKIT_HOME="$cache_dir"
                DIST_DIR="$SPECKIT_HOME/dist"
                warn "SPECKIT_HOME resolved from speckit.yml hub.url (cloned) → $SPECKIT_HOME"
                return 0
            fi
    ...
    for rule_file in "${rule_candidates[@]}"; do
        ...
        local hub_path
        hub_path=$(sed -n 's/.*\*\*Hub\*\*: `\([^`]*\)`.*/\1/p' "$rule_file" 2>/dev/null | head -1)
        if [[ -n "$hub_path" && "$hub_path" != *"SPECKIT_HUB_PATH"* && -d "$hub_path/dist" ]]; then
    ...
    err "dist/ not found at $SPECKIT_HOME"
    err ""
    err "If running from a consumer project, set the Hub in one of:"
    err "  1. .specify/config/speckit.yml  →  hub: { url: \"<git-url>\" }"
    err "  2. .cursor/rules/speckitsync_rule.mdc  →  **Hub**: \`/path/to/speckit\`"
...
check_version() {
    ...
    if [[ -z "$synced_remote_url" ]]; then
        # Fallback: hub.url from speckit.yml
...
main() {
    ...
    # Ensure SPECKIT_HOME points to actual Hub (with dist/), not consumer project root
    resolve_speckit_home
