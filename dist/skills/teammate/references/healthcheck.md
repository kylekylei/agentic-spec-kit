---
name: teammate/references/healthcheck
description: Teammate 工作流程健康診斷。執行 5 個 Pass 掃描 Foundation 完整性、階段合規、artifact 庫存、追溯骨架、Hub 同步狀態，產出 Healthcheck Report。
---

# Healthcheck

**定位**：工作流程健康檢查 —— 驗證 Teammate 流程是否被正確遵循。  
**操作限制**：嚴格唯讀，不修改任何檔案。

## 階段 0：定位任務

1. 從 repo 根目錄執行 `check-prerequisites.sh --json --paths-only`，解析 `TASK_DIR`
2. 若找不到 task → 掃描 `tasks/` 取得最近一個 task 目錄
3. 若完全找不到任務 → 只執行 Pass 1（Foundation）和 Pass 5（Hub Sync）

## Pass 1: Foundation Integrity

| 項目 | 檢查內容 |
|------|---------|
| `context.md` | 存在？有 `[ALL_CAPS_IDENTIFIER]` placeholder → CRITICAL；必要欄位（名稱/技術棧/使用者角色）是否填寫？ |
| `principles.md` | 存在？有 placeholder → CRITICAL；是否含至少一條 MUST/MUST NOT？ |
| `progress.md` | 存在？是否記載當前階段和下一步？ |
| `teammate.yml` | 存在？設定是否完整？ |

## Pass 2: Phase-Aware Artifact Inventory

依 `progress.md` 的當前階段判斷哪些 artifact 應存在：
...
**3b. Active Context 一致性** — `progress.md` 標記的階段是否與實際 artifact 吻合？
- progress 說 Execute 但 `plan.md` 不存在 → CRITICAL
- progress 說 Align 但 `plan.md` 已存在 → 可能未更新 progress
