...
1. 讀取 `SPEC_DIR/plan.md` Part 2，找到第一個 `- [ ]` action
2. 跳過所有 `- [x]`
...
1. **Setup**：從 repo 根目錄執行 `skills/speckit/scripts/bash/check-prerequisites.sh --json --require-plan --include-plan`並解析 `SPEC_DIR`、`AVAILABLE_DOCS`。
   - 參數含單引號時使用 escape 語法：`'I'\''m Groot'`

2. **檢查清單狀態**（若 `SPEC_DIR/checklists/` 存在）：掃描所有 checklist 檔案，統計完成/未完成。若有未完成 → 暫停詢問；全部完成 → 自動繼續。

3. **載入實作脈絡**：

   執行 `skills/speckit/scripts/bash/load-execution-context.sh --json` 並依輸出載入對應文件：
   - `required`：必定載入（plan.md、scenarios/*.feature）
   - `loaded`：存在即載入（insights、合約、llms.txt 等）
   - `compliance`：自動偵測到的合規 Skills（a11y / AI Risk）

   **Staleness Check**：若 `spec.md` 的修改時間比 `plan.md` 新 → 警告「plan 可能過期」，建議先執行 `/speckit.plan update`。
...
7. **執行 Red-Green 迴圈**：
   ...
   4. **VERIFY 階段**：
      ...
      a. 讀取 `speckit.yml` 的 `verification.test_command`（若為 null 則自動偵測框架）
   ...
   5. 先前 insight 需修正？
   6. 此 insight 是否在 3+ 個任務中重複出現？（→ 建議畢業至 `context.md` 或 `principles.md`，見 `@speckit` 洞察畢業機制）

   **規則**：
   - 每個 action **必須**寫入 `SPEC_DIR/insights.md`（有發現寫內容，無發現寫 `No new insights`）
...
11. **Update Active Context**：依 **Memory Delta Protocol**（見 `speckit-rules.mdc`）更新 `progress.md`：
...
12. **完成報告**：所有情境 GREEN + actions 完成 + 活文件已更新 → 建議 `/speckit.review`
