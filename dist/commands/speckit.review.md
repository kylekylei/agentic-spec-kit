...
**原則權威**：專案原則（`.specify/memory/principles.md`）**不可妥協**，原則衝突一律視為 CRITICAL。

**專業語氣**：客觀、建設性，適合團隊審查，禁止諷刺或煽動性用語。

### 階段 0：基礎檢查

1. **讀取 `.specify/memory/context.md`**
   - 掃描符合 `[ALL_CAPS_IDENTIFIER]` 的 placeholder token
   - 若發現 → **ERROR**："Project context not initialized. Run `/speckit.init` first."

2. **讀取 `.specify/memory/principles.md`**
   - 掃描符合 `[ALL_CAPS_IDENTIFIER]` 的 placeholder token
   - 若發現 → **ERROR**："Principles not defined. Run `/speckit.init` first."
   - **解析所有原則**：將每個 MUST / MUST NOT / SHOULD 規則及其 ID（如 `BB-001`、`III-D`）擷取至記憶體檢查清單，作為 Pass B3、Pass D Compliance、Pass G 的**權威參考**。

### 設定

從 repo root 執行 `skills/speckit/scripts/bash/check-prerequisites.sh --json --require-plan --include-plan`並解析：
...
推導路徑：
   ...
   - UI_SPEC = `SPEC_DIR/contracts/ui/ui-spec.md`（若存在）
   - INSIGHTS = `SPEC_DIR/insights.md`（若存在）
...
> **驗證機制**：若安裝測試框架（vitest/jest/pytest），將斷言寫入 `SPEC_DIR/checklists/review-assertions.spec.ts`；否則僅輸出為準備執行的斷言樣本。實現代碼通過所有斷言，即視為 Review 通過。
...
> 整合原 `/speckit.checklist` 的需求品質檢核功能。
...
依 `skills/speckit/references/review-report-format.md` 的格式規範，產生 `SPEC_DIR/checklists/feature-readiness.md`。
...
依發現結果：
- CRITICAL：必須先解決才能繼續 → 建議 `/speckit.plan update`
...
若使用者專案未安裝 design 分類的 skills（`.specify/config/skills.yml` 中無 design），跳過並在報告中標註「設計品質審查未啟用（未安裝設計 skills）」。
...
更新 `.specify/memory/milestone.md`：任務驗證狀態、覆蓋率指標、就緒評估。
