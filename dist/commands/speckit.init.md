...
`/speckit.init` 是**一鍵初始化**指令。它檢查並填寫兩個 Foundation 檔案，確保專案可以開始工作。

- **新專案**：引導使用者輸入，自動偵測 repo 資訊，建立 Foundation
- **既有專案**：檢視檔案完整性，分析是否有優化空間，報告狀態

> 整合原 `/speckit.kickoff` 和 `/speckit.principles` 的功能。
...
執行 `skills/speckit/scripts/bash/check-foundation.sh`並解析 JSON，報告目前狀態：
...
4. **複製 template 並填寫 `.specify/memory/context.md`**：
   - 若檔案不存在或為 template：從 `.specify/templates/context-template.md` 複製
   - 填寫以下區段：
...
3. **複製 template 並撰寫 `.specify/memory/principles.md`**：
   - 若檔案不存在或為 template：從 `.specify/templates/principles-template.md` 複製
   - 填入推導出的原則
...
Agents (4): speckit, designer, design-auditor, architect
...
> 事後管理 skills 請使用 `/speckit.skills` command。
...
- **Current State 專屬欄位**：Foundation Status: [完整/部分/缺失], Phase: Foundation, Last Command: init, Next Action: /speckit.align
