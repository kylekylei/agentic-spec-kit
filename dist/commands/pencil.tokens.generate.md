...
> ⚠️ 此步驟**不會**修改 `.pen` 檔案。`.pen` 為加密格式，必須透過 Pencil MCP 工具才能有效修改。
...
監聽 `tailwind.config.js` 變更，自動重新產生 `tokens-output.json`。每次更新後需手動執行 `/pencil.tokens.generate sync` 以同步至 Pencil。
...
> 第一個 value 不加 theme tag = Light 預設；第二個加 `theme: { "Mode": "Dark" }`。

### 新增 Token

編輯 `scripts/extractors/semantic-tokens.js`，加入新項目後執行 `/pencil.tokens.generate sync`。
...
| 問題 | 解法 |
|------|------|
| Tokens 未更新 | 確認 Step 2（`/pencil.tokens.generate sync`）有執行 |
