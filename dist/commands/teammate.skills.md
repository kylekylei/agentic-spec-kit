...
讀取 `.teammate/config/skills.yml` 和 `skill-registry.yml`，展示：
...
4. 使用 AskQuestion 確認
5. 更新 `.teammate/config/skills.yml`
6. 執行 `teammate-sync.sh`（selective mode）同步新增的資源
...
1. 讀取 `.teammate/config/skills.yml` 確認已安裝
...
- **Core 分類不可移除**（teammate, git-commit, code-review 等永遠必裝）
...
```
/teammate.skills sync         → 依 skills.yml 重新同步
/teammate.skills sync --check  → 檢查是否有新版本可用
```
