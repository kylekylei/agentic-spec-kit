#!/usr/bin/env bash
# install.sh — Install Teammate framework into a target project
#
# Usage:
...
TEAMMATE_HOME="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SYNC_SCRIPT="$TEAMMATE_HOME/dist/skills/teammate/scripts/bash/teammate-sync.sh"
...
check_prereqs() {
    ...
    if [[ ! -f "$SYNC_SCRIPT" ]]; then
        ...
        err "Make sure you are running install.sh from the Teammate repo root."
        exit 1
    fi
}
...
check_already_installed() {
    ...
    for f in "$target/.cursor/.teammate-sync-version" \
              "$target/.claude/.teammate-sync-version" \
              "$target/.agent/.teammate-sync-version"; do
    ...
    if [[ -n "$version_file" ]]; then
        ...
        warn "Teammate is already installed in $target (hash: $synced_hash)"
...
main() {
    ...
    # Parse args — pass through unknown flags to teammate-sync.sh
    while [[ $# -gt 0 ]]; do
        case "$1" in
            -h|--help) usage; exit 0 ;;
    ...
    echo ""
    echo "╔══════════════════════════════════════╗"
    echo "║        Teammate Framework Install     ║"
    echo "╚══════════════════════════════════════╝"
    echo ""
    info "Hub:    $TEAMMATE_HOME"
    info "Target: $target"
    echo ""
    ...
    # Delegate to teammate-sync.sh
    TEAMMATE_HOME="$TEAMMATE_HOME" bash "$SYNC_SCRIPT" "$target" "${extra_args[@]}"
